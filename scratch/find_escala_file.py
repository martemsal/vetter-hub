...
# 1. Checar pasta raiz compartilhada
print("Scanning Root...")
root_items = fetch_folder_items_deep('1hL6hQs1pqr7-sp0bdkU14CXJWe8RBcht', 'Root')
for item in root_items:
    ...
    if item['is_folder']:
        ...
        for s in sub_items:
            ...
            if s['is_folder']:
                deep_items = fetch_folder_items_deep(s['id'], f"{item['title']} / {s['title']}")
                for di in deep_items:
                    print(f"    [{'FOLDER' if di['is_folder'] else 'FILE'}] {di['title']} (ID: {di['id']})")
